...
router.post('/add-coins', async (req, res) => {
  ...
  await user.save();
  await CoinTransaction.create({ userId: user.id, amount, reason: reason || 'manual' });
  res.json({ user });
});
...
router.post('/claim-daily-bonus', async (req, res) => {
  try {
    ...
    if (lastLogin) {
      ...
      if (daysSinceLastLogin <= 1.5) { // Allow some grace period
    ...
    console.log(`💰 User ${user.name} claimed daily bonus: ${DAILY_LOGIN_COINS} coins (Streak: ${user.dailyLoginStreak})`);
...
router.post('/claim-ad-reward', async (req, res) => {
  try {
    ...
    console.log(`📺 User ${user.name} watched ad and earned ${AD_REWARD_COINS} coins`);
